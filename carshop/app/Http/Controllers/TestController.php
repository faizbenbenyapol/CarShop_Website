    ...
    public function search(Request $request)
{
    ...
    $keyword = $request->keyword; // รับค่าจากฟอร์ม

    if(strlen($keyword) > 0){
        // ค้นหาชื่อรถ
        $carList = TestModel::where('product_name', 'like', "%{$keyword}%")
                            ->paginate(12); // 12 รถต่อหน้า
    } else {
        // แสดงทั้งหมดเรียงล่าสุด
        $carList = TestModel::orderBy('id', 'desc')->paginate(12);
    }

    return view('test.shop', compact('carList', 'keyword'));
}
